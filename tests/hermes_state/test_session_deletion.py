...
# ---------------------------------------------------------------------------
# Origin's reset-fork policy (#114271, absorbed in the 2026-09-18 rebase): a
# row that rides a reset boundary starts its own user-visible conversation, so
# ``_is_compression_child_row`` now walks with ``include_reset=True`` and
# ``_NON_CONTINUATION_CHILD_FILTER_SQL`` gained ``_reset_from``. The delete CTE
# must answer the same way in BOTH directions:
# 1. a reset fork of a compression-ended parent survives (orphaned), and
# 2. a continuation that inherited a stale marker naming an EARLIER parent is
#    still deleted — markers are bound to ``parent_session_id``, never matched
#    by presence alone.
# ---------------------------------------------------------------------------


def test_delete_compression_chain_spares_reset_fork(db):
    """A reset fork is its own conversation even when its parent ended in
    compression, so deleting the chain must orphan it, not delete it.

    Deletes the ROOT on purpose: the CTE's descendant arm only enumerates a
    node's children when that node is in the walk, so a chain deleted from the
    tip never reaches the fork at all and would pass under any predicate.
    """
    base = time.time() - 100
    db.create_session("root", source="cli")
    db.create_session("tip", source="cli", parent_session_id="root")
    ...
    db._conn.execute(
        "UPDATE sessions SET started_at = ?, message_count = 1 WHERE id = 'reset'",
        (base + 30,),
    )
    db._conn.commit()
    ...
    # The compression chain is gone; the reset fork is orphaned, not deleted.
    assert db.get_session("root") is None
    assert db.get_session("tip") is None
    reset = db.get_session("reset")
    assert reset is not None
    assert reset["parent_session_id"] is None


def test_delete_compression_chain_keeps_continuation_with_foreign_marker(db):
    """A continuation copies its parent's ``model_config`` verbatim, so a marker
    naming an EARLIER parent must not disqualify it. Presence-only matching would
    drop the real continuation and leave an orphaned segment of the chain behind.

    The marker is ``_branched_from`` (not ``_reset_from``) because that is the
    marker a presence-only predicate tests: the regression is about HOW a marker
    is matched, not which marker exists.
    """
    base = time.time() - 200
    db.create_session("root", source="cli")
    db.create_session("mid", source="cli", parent_session_id="root")
    # ``tip`` still carries the branch marker it inherited from the chain root,
    # which does NOT name its own parent (``mid``) — it is a continuation, not a
    # fork, so binding (not presence) is what keeps it in the kill set.
    db.create_session(
        "tip", source="cli", parent_session_id="mid", model_config={"_branched_from": "root"},
    )
    db._conn.execute(
        "UPDATE sessions SET started_at = ?, ended_at = ?, end_reason = 'compression', message_count = 1 WHERE id = 'root'",
        (base, base + 10),
    ...
    # All three segments go: the foreign marker binds to ``root``, not to mid.
    assert db.get_session("root") is None
    assert db.get_session("mid") is None
    assert db.get_session("tip") is None